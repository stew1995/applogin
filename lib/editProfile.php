...
if(!empty($startingEmail) && ($startingPass)) {
  ...
  if(isset($_POST['submitName'])) {
    ...
    if(!empty($fname)) {
      ...
      $sql = "UPDATE user SET fname='$fname', lname='$lname' WHERE id='$getID'";
      mysqli_query($conn, $sql) or die("Cannot change first name and last name data in database" . mysqli_error($conn));
  ...
  if(isset($_POST['submitPassword'])){
    ...
    if($oldpassword = $firstPass && !empty($oldpassword) && !empty($firstPass)) {
      ...
      if($firstPass != $retypePass) {
        ...
        $sql = "UPDATE user SET password='$retypePass' WHERE fname='$getID'";
  ...
  if(isset($_POST['submitemail'])) {
    ...
    if($oldemail = $newemail && !empty($oldemail) && !empty($newemail)) {
      ...
      if ($newemail != $retype) {
        ...
        $sql = "UPDATE user SET email='$newemail' WHERE fname='$getID'";
  ...
  if(isset($_POST['submitAddress'])) {
    ...
    if(!empty($address)){
      $sql = "UPDATE user SET address='$address' WHERE fname='$getID'";
  ...
  if(isset($_POST['submitpostcode'])) {
    ...
    if(!empty($postcode)){
      $sql = "UPDATE user SET postcode='$postcode' WHERE fname='$getID";
  ...
  if(isset($_POST['submitmobile'])) {
    ...
    if(!empty($newmobile)) {
      $sql = "UPDATE user SET mobile='$newmobile' WHERE fname='$getID'";
      mysqli_query($conn, $sql) or die("Cannot change mobile data in database" . mysqli_error($conn));

    }

    if(!empty($newhome)) {
      $sql = "UPDATE user SET homemobile='$newhome' WHERE fname='$getID'";
  ...
  if(isset($_POST['submithobbie'])) {
    ...
    if(!empty($hobbie)) {
      $sql ="UPDATE user SET hobbie='$searchID' WHERE fname='$getID'";
  ...
  if(isset($_POST['submitsociety'])) {
    ...
    if(!empty($society)) {
      $sql = "UPDATE user SET society='$searchID' WHERE fname='$getID'";
  ...
  if(isset($_POST['submitcourse'])) {
    ...
    if(!empty($course)) {
      $sql = "UPDATE uni SET course='$getID' WHERE fname='$getID'";
  ...
  if(isset($_POST['submitaccom'])) {
    ...
    if(!empty($accom)) {
      $sql = "UPDATE uni SET accom='$accom' WHERE fname='$getID'";
  ...
  if(isset($_POST['submitsmoke'])) {
    $smoke = $_POST['smoke'];
    if(!empty($smoker)) {
      $sql = "UPDATE user SET smoker='$smoke' WHERE fname='$getID'";
